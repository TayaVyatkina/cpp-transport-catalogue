...
RequestHandler::RequestHandler(transport_catalogue::TransportCatalogue* data_base, renderer::MapRenderer* renderer, graph::TransportRouter* router)
    : data_base_(data_base)
    , map_renderer_(renderer)
    , router_(router)
{}

RequestHandler::RequestHandler(transport_catalogue::TransportCatalogue* data_base)
    : data_base_(data_base)
{}

void RequestHandler::AddDataBase(transport_catalogue::TransportCatalogue* data_base) {
    data_base_ = data_base;
}

void RequestHandler::AddMapRenderer(renderer::MapRenderer* renderer) {
    map_renderer_ = renderer;
}

void RequestHandler::AddTransportRouter(graph::TransportRouter* router) {
    router_ = router;
}
...
std::optional<information_base::ReportRouter> RequestHandler::GetReportRouter(const std::string_view& from, const std::string_view& to) const {
